...
document.addEventListener('click',async(e)=>{
  ...
  if(view){e.preventDefault();e.stopPropagation();openPDF(view)}
});
...
function triggerDownloadFromBlob(blob,filename){
  ...
  a.href=href;a.download=filename;a.style.position='fixed';a.style.left='-9999px';
  document.body.appendChild(a);a.click();a.remove();setTimeout(()=>URL.revokeObjectURL(href),4000);
}
function fallbackLink(url,filename){
  const a=document.createElement('a');a.href=url;a.setAttribute('download',filename);
  a.style.position='fixed';a.style.left='-9999px';document.body.appendChild(a);a.click();a.remove();
}
