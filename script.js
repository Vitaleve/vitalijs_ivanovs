...
(function setCanonicalAndLD(){
  const url=location.origin+location.pathname;
  const links=document.querySelectorAll('link[rel="canonical"],link[rel="alternate"]');
  links.forEach(l=>l.setAttribute('href',url));
  const ld=document.querySelector('script[type="application/ld+json"]');
  if(ld){try{const data=JSON.parse(ld.textContent);data["@graph"]?.forEach(n=>{if(n.url!==undefined)n.url=url;if(n.potentialAction&&n.potentialAction.target!==undefined)n.potentialAction.target=url});ld.textContent=JSON.stringify(data)}catch(_){}}
})();
...
function applyTheme(val){
  ...
  setTimeout(()=>document.documentElement.classList.remove("theme-changing"),120);
}
...
const I18N={
  de:{title:"Vitalijs Ivanovs Praktikum",nav:["Über mich","Lebenslauf","Unterlagen","Projekte","Kontakt"],metaDesc:"Bewerbung, Lebenslauf, Projekte, Zeugnisse.",ogDesc:"Lebenslauf, Anschreiben, Projekte, Zertifikate. Praktikum 2026.",heroWord:"BEWERBUNG",heroSubtitle:"Pflichtpraktikum als Fachinformatiker für Anwendungsentwicklung",heroCta:"Unterlagen ansehen",qf_docs:"📄 Unterlagen",qf_contact:"📧 Kontakt",modal_close:"Schließen",qr_alt:"QR-Code zum Bewerbungsportal"},
  en:{title:"Vitalijs Ivanovs Praktikum",nav:["About me","CV","Documents","Projects","Contact"],metaDesc:"Application, résumé, projects, certificates.",ogDesc:"Résumé, cover letter, projects, certificates. Internship 2026.",heroWord:"APPLICATION",heroSubtitle:"Mandatory internship as IT specialist for application development",heroCta:"View documents",qf_docs:"📄 Documents",qf_contact:"📧 Contact",modal_close:"Close",qr_alt:"QR code to the application page"}
};
...
function setMeta(lang){
  ...
  if(ogd)ogd.setAttribute("content",t.ogDesc);
  if(twd)twd.setAttribute("content",t.ogDesc);
}
...
if(copyBtn&&mailLink){
  copyBtn.addEventListener("click",async()=>{
    try{
      await navigator.clipboard.writeText(mailLink.textContent.trim());
      copyBtn.classList.add("copied");
      const tip=document.getElementById("copy-tip"); if(tip) tip.textContent="Kopiert!";
      setTimeout(()=>{copyBtn.classList.remove("copied"); if(tip) tip.textContent="In die Zwischenablage";},1600);
    }catch(_){}
  });
}
